...
Page ${page} of ${pages}
${page > 1 ? html`<a class="pager" href=${'/catalog/' + (page - 1) + (search ? `?search=${search}` : '')}>&lt; Prev</a>` : ''}
${page < pages ? html`<a class="pager" href=${'/catalog/' + (page + 1) + (search ? `?search=${search}` : '')}>Next &gt;</a>` : ''}`;
